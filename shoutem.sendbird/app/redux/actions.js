...
export const CHANNEL_MESSAGES_LOADED =
  'shoutem.sendbird.CHANNEL_MESSAGES_LOADED';
...
export const CONNECTION_STATUS_CHANGE =
  'shoutem.sendbird.CONNECTION_STATUS_CHANGE';
...
export function loadChannelMessages(channel, messageQuery, append = false) {
  return dispatch =>
    SendBird.getChannelMessages(messageQuery).then(messages =>
      dispatch({
...
export function searchChannelsPerNickname(nickname) {
  return dispatch =>
    SendBird.searchConversationsPerNickname(nickname, 3).then(channels =>
      dispatch({
...
export function sendMessage(channel, message) {
  ...
  return dispatch =>
    SendBird.sendMessage(message, channel).then(newMessage =>
      dispatch({
        type: NEW_MESSAGE_ADDED,
        payload: { message: newMessage, channelId },
      }),
...
export function loadChannels(groupChannelListQuery, append = false) {
  return dispatch =>
    SendBird.getUserChatList(groupChannelListQuery).then(channels =>
      dispatch({
...
export function loadChannel(channelId) {
  return dispatch =>
    SendBird.getChannelPerId(channelId).then(channel =>
      dispatch({
...
export function createChannel(userId, targetId) {
  return dispatch =>
    SendBird.createDirectChat(userId, targetId).then(channel => {
