...
function getResizeRatio(pageDimensions, referentResolution) {
  const widthRatio = calculateSizeRatio(
    pageDimensions.width,
    referentResolution.width,
  );
  const heightRatio = calculateSizeRatio(
    pageDimensions.height,
    referentResolution.height,
  );
...
export default class Scaler {
  ...
  resolveRatioByWidth(pageDimensions, referentResolution) {
    return this.setRatio(
      calculateSizeRatio(pageDimensions.width, referentResolution.width),
    );
  }

  resolveRatioByHeight(pageDimensions, referentResolution) {
    return this.setRatio(
      calculateSizeRatio(pageDimensions.height, referentResolution.height),
    );
  }
