...
function mapStateToProps(state, ownProps) {
  ...
  const extensionName = _.get(context, 'extensionName');

  const extensionLocale = _.get(context, 'i18n.locale');
  const extensionTranslationUrl = _.get(context, 'i18n.translationUrl');

  const dependency = _.get(context , `i18n.dependencies.${ownExtensionName}`);
